...
export default function PricingCalculator() {
  ...
  const saveEstimate = async () => {
    ...
    try {
      ...
      window.open(`${CAL_LINKS.ale}?email=${encodeURIComponent(email)}`, '_blank');
    } catch (error) {
      ...
      <div className="bg-gradient-to-br from-slate-900/50 to-slate-800/30 backdrop-blur-sm border border-white/10 rounded-2xl p-6 sticky top-24">
        ...
        <div className="space-y-4">
          {breakdown.map((section) => (
            <div key={section.stepTitle} className="border-b border-white/10 pb-3 last:border-0">
              <div className="flex items-center justify-between mb-2">
                <span className="text-sm font-semibold text-cyan-400">{section.stepTitle}</span>
                {section.total > 0 && (
                  <span className="text-sm text-white">${section.total.toLocaleString()}</span>
                )}
              </div>
  ...
  if (showSummary) {
    ...
    const timelineSelection = selections[4] ? PRICING_STEPS[4].options.find(opt => opt.id === selections[4][0]) : null;
            ...
            <div className="bg-gradient-to-br from-slate-900/50 to-slate-800/30 backdrop-blur-sm border border-white/10 rounded-2xl p-8">
              ...
              {timelineSelection && timelineSelection.id !== 'standard' && (
                <div className="mt-6 p-4 bg-purple-500/10 border border-purple-500/30 rounded-lg">
                  ...
                  <p className="text-sm text-gray-400 mt-1 ml-7">{timelineSelection.description}</p>
                </div>
              )}
            </div>
